...
declare -a MANIFEST_CHECKS=(
    ...
    "utils.openproxy	OpenProxy - lightweight HTTP/HTTPS proxy server	command -v openproxy	optional	target_user"
    "utils.fast_file_search	Fast File Search - rapid file discovery tool	command -v fast_file_search	optional	target_user"
    "utils.rtco	rtco (Rust Token Cost Optimizer) - LLM output filter and context management for AI coding agents	command -v rtco	optional	target_user"
    "utils.hashline	hashline - stable line-addressed file editing using content-hashed anchors	command -v hashline	optional	target_user"
